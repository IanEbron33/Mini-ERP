...
function SalesOrdersContent() {
  ...
  const handleCreateOrder = async (e: React.FormEvent) => {
    ...
    if (availableStock <= 0) {
      toast.error(`"${currentSelectedProduct.name}" is currently out of stock!`);
      return;
    }
    ...
    if (finalQty > availableStock) {
      toast.error(`Quantity (${finalQty}) exceeds available stock (${availableStock} units remaining).`);
      return;
    }
    ...
    if (res.success && res.order) {
      toast.success(`Sales Order ${res.order.order_number} created successfully!`);
    ...
    <div className="space-y-6">
          ...
          <div>
            <div className="flex items-center gap-2 mb-1">
              <Badge className="bg-emerald-50 text-emerald-800 border-emerald-200 text-[10px] uppercase font-bold tracking-wider px-2 py-0.5">
                Full Read/Write Sales Workspace
              </Badge>
              <Badge className="bg-[#fff7e8] text-[#713105] border-[#e8decf] text-[10px] font-semibold">
            ...
            <p className="text-xs font-normal text-[#7f5e35] mt-1">
              Create new sales orders, automatically verify real-time stock, deduct inventory, generate branded invoices, and manage payment states.
            </p>
          </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
        <Card className="border-[#e8decf] bg-white p-4 rounded-xl shadow-xs">
          ...
          <span className="text-[11px] text-[#7f5e35] font-normal">Active transactions</span>
        </Card>
        ...
        <Card className="border-[#e8decf] bg-white p-4 rounded-xl shadow-xs">
          ...
          <span className="text-[11px] text-[#713105] font-semibold">Requires fulfillment</span>
        </Card>
        ...
        <Card className="border-[#e8decf] bg-white p-4 rounded-xl shadow-xs">
          ...
          <span className="text-[11px] text-[#7f5e35] font-normal">Per order record</span>
        </Card>
      </div>
      ...
      <Card className="border-[#e8decf] shadow-xs rounded-xl bg-white overflow-hidden">
        <CardHeader className="p-5 border-b border-[#e8decf] flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
          <CardTitle className="text-sm font-semibold text-[#4f351c]">
            Live Orders Directory & Invoicing Records
          </CardTitle>
          ...
          <table className="w-full text-left text-xs text-[#341100]">
            <thead className="bg-[#fff7e8] border-b border-[#e8decf] text-[11px] uppercase tracking-wider text-[#7f5e35] font-semibold">
              <tr>
                ...
                <th className="py-3 px-4">Item Units</th>
              ...
              {isLoading ? (
                [...Array(5)].map((_, i) => (
                  <tr key={i} className="animate-pulse">
                    <td className="py-3.5 px-4"><div className="w-20 h-3.5 bg-[#e8decf]/70 rounded-md" /></td>
                    <td className="py-3.5 px-4"><div className="w-28 h-3.5 bg-[#e8decf]/60 rounded-md" /></td>
                    ...
                    <td className="py-3.5 px-4"><div className="w-16 h-3.5 bg-[#e8decf]/70 rounded-md" /></td>
                    ...
                    <td className="py-3.5 px-4 text-right"><div className="w-24 h-6 bg-[#e8decf]/60 rounded-lg ml-auto" /></td>
                  </tr>
                ))
                    ...
                    <div className="max-w-xs mx-auto space-y-2">
                      ...
                      <p className="text-[11px] text-[#7f5e35]">
                        {searchTerm || statusFilter !== "All"
                          ...
                          : "No sales orders placed yet. Click 'Create Sales Order' to start."}
                      </p>
                    </div>
                  </td>
                </tr>
        ...
        <div className="fixed inset-0 bg-stone-950/40 backdrop-blur-xs flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-3xl border border-[#e8decf] shadow-2xl w-full max-w-lg overflow-hidden animate-in fade-in zoom-in-95">
            ...
            <form onSubmit={handleCreateOrder} className="p-6 space-y-4 text-xs text-[#341100]">
              ...
              {/* Product Stock & Pricing Details Pill */}
...
export function SalesOrdersPage() {
  return (
    <Suspense fallback={<div className="p-6 text-xs text-[#7f5e35]">Loading Sales Workspace...</div>}>
      <SalesOrdersContent />
    </Suspense>
  );
}
